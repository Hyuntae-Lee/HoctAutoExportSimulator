...
int Patient::getAge(void)
{
	COleDateTime current = COleDateTime::GetCurrentTime();
	int age = current.GetYear() - m_birthDate.GetYear();

	if (m_birthDate.GetMonth() > current.GetMonth())
	{
		age -= 1;
	}
	else if (m_birthDate.GetMonth() == current.GetMonth() && m_birthDate.GetDay() > current.GetDay())
	{
		age -= 1;
	}
